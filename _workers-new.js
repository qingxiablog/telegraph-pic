...
const FORUM_SESSION_COOKIE = '__Host-forum_session';
const FORUM_STATE_COOKIE = '__Host-forum_oauth_state';
const FORUM_STATE_TTL = 600;
...
async function sealSession(value, secret) {
...
function forumOauthConfig(env) {
  // Required secrets/URLs: FORUM_OAUTH_AUTHORIZE_URL, FORUM_OAUTH_TOKEN_URL,
  // FORUM_OAUTH_USERINFO_URL, FORUM_OAUTH_CLIENT_ID, FORUM_OAUTH_CLIENT_SECRET,
  // FORUM_OAUTH_REDIRECT_URI (exactly registered in the forum), and
  // FORUM_OAUTH_SESSION_SECRET (a random 32+ character secret).
  return {
    enabled: env.FORUM_OAUTH_ENABLED !== 'false',
    authorizeUrl: env.FORUM_OAUTH_AUTHORIZE_URL || '',
    tokenUrl: env.FORUM_OAUTH_TOKEN_URL || '',
    userinfoUrl: env.FORUM_OAUTH_USERINFO_URL || '',
    revokeUrl: env.FORUM_OAUTH_REVOKE_URL || '',
    clientId: env.FORUM_OAUTH_CLIENT_ID || '',
    clientSecret: env.FORUM_OAUTH_CLIENT_SECRET || '',
    redirectUri: env.FORUM_OAUTH_REDIRECT_URI || '',
    sessionSecret: env.FORUM_OAUTH_SESSION_SECRET || ''
  };
}

function secureOAuthUrl(value) {
  try {
...
function forumOauthConfigured(config) {
  return config.enabled && [
    secureOAuthUrl(config.authorizeUrl),
    secureOAuthUrl(config.tokenUrl),
    secureOAuthUrl(config.userinfoUrl),
    secureOAuthUrl(config.redirectUri),
    config.clientId,
    config.clientSecret,
...
function forumOauthRedirectUri(request, config) {
  return config.redirectUri || new URL('/auth/callback', request.url).toString();
}

async function forumOauthUser(request, config) {
  if (!forumOauthConfigured(config)) return null;
  const session = await openSession(getCookie(request, FORUM_SESSION_COOKIE), config.sessionSecret);
  if (!session || typeof session.accessToken !== 'string' || Number(session.expiresAt) <= Math.floor(Date.now() / 1000)) return null;
  try {
    ...
    return user && user.sub ? user : null;
  } catch {
    return null;
  }
}

function authRequiredResponse(config, message = '') {
  const error = message ? `<p class="error">${escapeHtml(message)}</p>` : '';
  const missing = forumOauthConfigured(config) ? '' : '<p class="error">Worker 尚未配置完整的论坛 OAuth 环境变量。</p>';
  return new Response(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>休息图床</title><style>body{font-family:system-ui,sans-serif;display:grid;place-items:center;min-height:100vh;margin:0;background:#f5f6fa}.card{background:#fff;padding:32px;border-radius:16px;box-shadow:0 8px 30px #0001;text-align:center;max-width:360px}h2{margin:0 0 20px}a{display:inline-block;padding:10px 18px;border-radius:8px;background:#667eea;color:#fff;text-decoration:none}.error{color:#c62828}</style><main class="card"><h2>休息图床</h2><p>上传文件须使用休息社区授权</p>${error}${missing}${forumOauthConfigured(config) ? '<a href="/auth/login">使用 XIUXI.XYZ 登录</a>' : ''}</main></html>`, {
    status: 401,
    headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' }
  });
}

async function forumOauthLogin(request, config) {
  if (!forumOauthConfigured(config)) return jsonResponse({ error: '论坛 OAuth 配置不完整' }, 503);
  ...
  const stateCookie = await sealSession({ state, verifier, expiresAt: Math.floor(Date.now() / 1000) + FORUM_STATE_TTL }, config.sessionSecret);
  const params = new URLSearchParams({
    response_type: 'code',
    client_id: config.clientId,
    redirect_uri: forumOauthRedirectUri(request, config),
    scope: 'profile',
    state,
  ...
  return new Response(null, {
    status: 302,
    headers: {
      Location: `${config.authorizeUrl}${config.authorizeUrl.includes('?') ? '&' : '?'}${params}`,
      'Set-Cookie': cookieHeader(FORUM_STATE_COOKIE, stateCookie, FORUM_STATE_TTL, requestIsSecure(request)),
      'Cache-Control': 'no-store'
    }
  });
}

async function forumOauthCallback(request, config) {
  if (!forumOauthConfigured(config)) return jsonResponse({ error: '论坛 OAuth 配置不完整' }, 503);
  const url = new URL(request.url);
  const state = url.searchParams.get('state') || '';
  const code = url.searchParams.get('code') || '';
  const stateData = await openSession(getCookie(request, FORUM_STATE_COOKIE), config.sessionSecret);
  const invalidState = !stateData || stateData.state !== state || Number(stateData.expiresAt) < Math.floor(Date.now() / 1000);
  if (invalidState || !code) return authRequiredResponse(config, '授权回调已失效，请重新登录。');
  ...
  try {
    ...
    if (!tokenResponse.ok) return authRequiredResponse(config, '论坛令牌交换失败，请重新登录。');
    const token = await tokenResponse.json();
    const accessToken = typeof token.access_token === 'string' ? token.access_token : '';
    if (!accessToken) return authRequiredResponse(config, '论坛没有返回有效令牌。');
    const userResponse = await fetch(config.userinfoUrl, { headers: { Authorization: `Bearer ${accessToken}`, Accept: 'application/json' } });
    if (!userResponse.ok) return authRequiredResponse(config, '论坛身份验证失败，请重新登录。');
    const user = await userResponse.json();
    if (!user || !user.sub) return authRequiredResponse(config, '论坛返回的身份无效。');
    const expiresIn = Math.max(60, Math.min(Number(token.expires_in) || 3600, 3600));
    const session = await sealSession({ accessToken, expiresAt: Math.floor(Date.now() / 1000) + expiresIn }, config.sessionSecret);
    const headers = new Headers({ Location: '/', 'Cache-Control': 'no-store' });
    headers.append('Set-Cookie', cookieHeader(FORUM_SESSION_COOKIE, session, expiresIn, requestIsSecure(request)));
    headers.append('Set-Cookie', cookieHeader(FORUM_STATE_COOKIE, '', 0, requestIsSecure(request)));
    return new Response(null, { status: 302, headers });
  } catch (error) {
    console.error('论坛 OAuth 回调失败:', error && error.message);
    return authRequiredResponse(config, '论坛暂时不可用，请稍后重试。');
  }
}

async function forumOauthLogout(request, config) {
  const session = await openSession(getCookie(request, FORUM_SESSION_COOKIE), config.sessionSecret);
  if (session && session.accessToken && config.revokeUrl && forumOauthConfigured(config)) {
    try {
  ...
  const headers = new Headers({ Location: '/', 'Cache-Control': 'no-store' });
  headers.append('Set-Cookie', cookieHeader(FORUM_SESSION_COOKIE, '', 0, requestIsSecure(request)));
  return new Response(null, { status: 302, headers });
}
...
function extractConfig(env) {
  return {
    ...
    forumOauth: forumOauthConfig(env)
  };
}
      ...
      switch (pathname) {
        case '/auth/login':
          return request.method === 'GET'
            ? await forumOauthLogin(request, config.forumOauth)
            : new Response('Method Not Allowed', { status: 405 });
        case '/auth/callback':
          return request.method === 'GET'
            ? await forumOauthCallback(request, config.forumOauth)
            : new Response('Method Not Allowed', { status: 405 });
        case '/auth/logout':
          return (request.method === 'GET' || request.method === 'POST')
            ? await forumOauthLogout(request, config.forumOauth)
            : new Response('Method Not Allowed', { status: 405 });
        case '/':
...
async function handleRootRequest(request, config) {
  if (config.forumOauth.enabled && !(await forumOauthUser(request, config.forumOauth))) {
    return authRequiredResponse(config.forumOauth);
  }
  if (!config.forumOauth.enabled && config.enableAuth && !authenticate(request, config.username, config.password)) {
...
<meta name="description" content="休息图床-基于Workers的图床服务">
<meta name="keywords" content="休息图床,Workers图床,Telegram,图床,Cloudflare,Workers">
<title>休息图床-基于Workers的图床服务</title>
...
<body>
  ...
  <div class="card">
    <h2 class="title">休息图床</h2>
    <div style="margin:-12px 0 16px;color:#777;font-size:12px">已通过论坛授权 · <a href="/auth/logout" style="color:#667eea">退出登录</a></div>
    ...
    <p class="project-link">
      休息图床 -
      <a href="https://github.com/0-RTT/telegraph" target="_blank" rel="noopener noreferrer">Powerd By 0-RTT/telegraph</a>
    </p>
  </div>
  ...
  `;
  const response = config.forumOauth.enabled
    ? new Response(html, { headers: { 'Content-Type': 'text/html;charset=UTF-8', 'Cache-Control': 'no-store' } })
    : createCachedResponse(html, 'text/html;charset=UTF-8', CACHE_CONFIG.HTML);

  if (!config.forumOauth.enabled) await cache.put(cacheKey, response.clone());
  return response;
}
...
async function handleUploadRequest(request, config) {
  if (config.forumOauth.enabled) {
    ...
    if (!(await forumOauthUser(request, config.forumOauth))) {
      return jsonResponse({ error: '请先完成论坛授权' }, 401);
    }
  } else if (config.enableAuth && !authenticate(request, config.username, config.password)) {
    return unauthorizedResponse();
  }
